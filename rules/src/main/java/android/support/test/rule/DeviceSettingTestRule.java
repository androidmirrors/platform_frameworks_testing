...
public class DeviceSettingTestRule implements TestRule {
  ...
  public static final String TAG = DeviceSettingTestRule.class.getName();
  ...
  @NonNull
  private Statement deviceSettingApplierStatement(
      final WithSetting settingAnnotation, final Statement base) {
    return new Statement() {
      ...
      public void evaluate() throws Throwable {
        ...
        try {
          setting = settingAnnotation.value().newInstance();
          setting.apply();
          isApplied = true;
          base.evaluate();
        }catch (SecurityException exception) {
          Log.e(TAG, exception.getMessage());
          throw new PermissionDenialException(exception.getMessage());
        }
        finally {
          if (isApplied) {
            setting.reset();

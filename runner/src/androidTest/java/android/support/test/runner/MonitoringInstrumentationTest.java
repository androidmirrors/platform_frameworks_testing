...
import android.support.test.internal.util.InterceptingActivityFactory;
...
public class MonitoringInstrumentationTest {
    ...
    @Test
    public void shouldNotCreateNewActivityUsingInterceptingActivityFactoryIfReset()
        throws Exception {
        final Class<TestActivity> testActivityClass = TestActivity.class;
        final AtomicReference<Activity> activity = new AtomicReference<>();
    ...
    private void retrieveActivityOnMainThread(final Class<TestActivity> activityClass,
                                          final AtomicReference<Activity> activityAtomicReference) {
        instrumentation.runOnMainSync(new Runnable() {
            @Override
            public void run() {
                try {
                    activityAtomicReference.set(instrumentation.newActivity(
    ...
    @NonNull
    private <T extends Activity> InterceptingActivityFactory interceptingActivityFactory(
        final T activity, final boolean shouldCreate) {
        return new InterceptingActivityFactory() {
            @Override
            public boolean shouldIntercept(ClassLoader classLoader, String className,
                                           Intent intent) {
                return shouldCreate;
            }
